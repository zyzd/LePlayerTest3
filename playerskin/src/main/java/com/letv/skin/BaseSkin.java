...
import com.letv.skin.v4.ARightSettingMenu;
...
public abstract class BaseSkin extends BaseViewGourp implements IPlaySkin, Observer, OnClickListener {
    ...
    private void addContainer() {
        videoViewContainer = new RelativeLayout(context);//播放器视图
        LayoutParams params = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        params.addRule(RelativeLayout.CENTER_IN_PARENT);
        addView(videoViewContainer, params);

        controllerContainer = new RelativeLayout(context);//控制器视图
    ...
    public void update(Observable observable, Object data) {
        ...
        switch (state) {
            ...
            case ISplayerController.SCREEN_ORIENTATION_USER_LANDSCAPE:
                ...
                break;
            case 10086:
//                Log.d("zyzd", "update:____________________________");
//                changeBottom();
                break;
            default:
                break;
    ...
    private boolean isShowUserFloating;

    public boolean isShowUserFloating() {
        return isShowUserFloating;
    }

    public void setShowUserFloating(boolean showUserFloating) {
        isShowUserFloating = showUserFloating;
    }
    ...
    public void onClick(View v) {
        ...
        if (isShowUserFloating) {
            EventBus.getDefault().post(ConstantUtils.EVENT_PANO_SETTINGMENU_GONE);//备注：这个关键点：必须和Constant.EVENT_PANO_SETTINGMENU_GONE值一致
            isShowUserFloating = false;
            return;
        }
    ...
    private void changeBottom() {
        for (int i = 0; i < childViews.size(); i++) {
            ...
            if (baseView instanceof V4LargeMediaController) {
                return;
            }
        }
    }
    ...
    private void showFloatingView() {
        ...
        for (int i = 0; i < childViews.size(); i++) {
            ...
            if (baseView instanceof ARightSettingMenu) {
                continue;
            }
