...
public class LetvBaseHelper {
    ...
    public void init(Context mContext, Bundle mBundle, V4PlaySkin skin) {
        ...
        this.skin = skin;//传入皮肤
        this.mBundle = mBundle;//传入参数
        playMode = mBundle.getInt(PlayProxy.PLAY_MODE, -1);//取得播放模式
        switch (playMode) {//根据播放模式选择对应的皮肤类型
    ...
    private void initPlayContext() {
        ...
        playContext.setVideoContainer(skin);//Container：容器 ，设置皮肤
        playContext.setUsePlayerProxy(USE_PLAYER_PROXY);//设置播放器代理
...
//    }
    ...
    protected void handlePlayState(int state, Bundle bundle) {
        switch (state) {
            case ISplayer.MEDIA_EVENT_PREPARE_COMPLETE://准备完成时
                // 获取当前播放的码率
                uicontext.setCurrentRateType(playContext.getCurrentDefinationType());
                if (uicontext.isClickPauseByUser() && player != null) {//当被用户点击暂停时，调整至静音
                ...
                break;
            case ISplayer.MEDIA_EVENT_FIRST_RENDER://TODO 第一次播放（未确定）
                if (renderCallback != null) {
                    renderCallback.onRender();
                }
                if (uicontext.isClickPauseByUser() && player != null) {//由用户点击的暂停，发送延迟信息
                    player.pause();
            ...
            case ISplayer.MEDIA_EVENT_VIDEO_SIZE:
                ...
                break;
            case EventPlayProxy.PLAYER_ACTION_LIVE_STATUS://直播状态
